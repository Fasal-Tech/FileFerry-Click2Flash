 ...
 *
 */
static const char* const gcFilesNamesTable[eFS_MAX] = {[eFS_GOLDEN_IMAGE] = "fallback.txt", [eFS_FILE_2] = "file2.txt"};
 ...
 * @return eStorageFSStatus_t
 */
static eStorageFSStatus_t FlashFs_ReadFileRaw(sFlashFS_t* const pMe, eStorageFileNamesEnums_t fileEnum, char* const pOutReadBuf, uint32_t bytesToRead,
                                              int32_t* const pOutBytesRead)
{
    assert(NULL != pMe);
 ...
 * @return eStorageFSStatus_t
 */
static eStorageFSStatus_t FlashFs_WriteFileRaw(sFlashFS_t* const pMe, eStorageFileNamesEnums_t fileEnum, bool IsAppend, const char* const pInWriteBuf,
                                               size_t bufSize)
{
    assert(NULL != pMe);
    assert(fileEnum < eFS_MAX);
 ...
 * @return eStorageFSStatus_t
 */
static eStorageFSStatus_t FlashFs_ComputeFileCRC(sFlashFS_t* const pMe, eStorageFileNamesEnums_t fileEnum, uint8_t* const pInOutRamBuf, uint32_t RamBufSize,
